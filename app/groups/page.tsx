...
export default async function GroupsPage() {
  ...
  return (
    <>
      ...
      <main
        id="main-content"
        className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-10 px-5 py-12"
      >
        <section className="flex flex-col gap-4">
          <h1 className="text-2xl font-semibold">Your groups</h1>
          {groups.length === 0 ? (
            <p className="text-[var(--muted)]">
              You are not in a group yet. Create one, or join with an invite
              code from an organizer.
            </p>
          ) : (
